			...
			foreach($this->db_values[$type] as $values)
			{
				...
				switch ($input_type[0])
				{
					...
					case 'radio':
						...
						{
							$vals = explode('^',$value);
							$input_field .= '<label class="'.( $values['value'] == $vals[1] ? 'blue' : 'white' ).'"><input class="checkBox" type="radio" name="config_'.str_replace(' ', '++', $values['name']).'" value="'.$vals[1].'" '.( $values['value'] == $vals[1] ? 'checked="checked"' : '' ).' />'.$vals[0]."</label>\n";
						}
						break;
					...
					case 'selectm':
						$options = explode('|',$input_type[1]);
						$input_field .= '<select multiple name="config_'.$values['name'].'">'."\n";
						$select_one = 1;
						foreach( $options as $value )
						{
							$vals = explode('^',$value);
