...
* -----------------------------
*
* $Id: RecipeList.php 18 2006-12-27 06:24:38Z zanix $
*
******************************/
	...
	if( isset( $recipes[0] ) )
	{
		...
		foreach ($recipes as $recipe)
		{
			if ($recipe_type != $recipe->data['recipe_type'])
			{
				...
				{
					$content .=  '<th class="membersHeader"><a href="?roster_addon_name='.$_GET['roster_addon_name'].'&amp;proffilter='.$_REQUEST["proffilter"].'&amp;sort=type">'.$wordings[$roster_conf['roster_lang']]['name'].'</a></th>'."\n";
				}
				...
				{
					$content .=  '<th class="membersHeader"><a href="?roster_addon_name='.$_GET['roster_addon_name'].'&amp;proffilter='.$_REQUEST["proffilter"].'&amp;sort=level'.$sort_d.'">&nbsp;'.$wordings[$roster_conf['roster_lang']]['level'].'&nbsp;</a></th>'."\n";
				}
			...
			while($row_users = $wowdb->fetch_array($result_users))
			{
				...
				if (substr($row_users['skill_level'],0,strpos($row_users['skill_level'],':')) < 375)
				{
...
"<a href='?roster_addon_name=".$_GET['roster_addon_name']."&amp;action=config'>&nbsp;&nbsp;".$wordings[$roster_conf['roster_lang']]['MadeBy_Configure_txt']."&nbsp;&nbsp;</a>"
