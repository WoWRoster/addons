...
class ArmorySync extends ArmorySyncBase {
    ...
    function _getGuildInfo() {
        ...
        $content = $this->getguilddata( $roster->data['guild_name'], $this->region, $this->server, $fetch_type='array' );//$armory->fetchGuild( $this->memberName, $roster->config['locale'], $this->server );
    ...
    function _getCharacterInfo() {
		...
		$this->_setUserAgent($armory);
            ...
            $content = $this->getCharacterData($realm, $name);//$this->_parseData( $armory->fetchCharacter( $this->memberName, $roster->config['locale'], $this->server ) );
     ...
     */
      function _getReputationInfo() 
      {
		...
		else
		{
            ...
            $content =  $this->getCharacterRep($base_url, $this->server,$this->memberName);    
    ...
    function _getTalentInfo() 
    {
		...
		else
		{
            ...
            $content = $this->getTalentData($base_url, $this->server, $this->memberName );

            foreach ($content->characterInfo->talentTab->talentTree as $val)
            {
		...
		$this->status['talentInfo'] = 0;
            ...
            $this->_debug( 1, true, 'Char: '. $this->memberName. ' ('.$clas.') Parsed talent info', 'OK' );
		return true;
    }
    // Helper functions
...
{
    $url = $this->_makeUrl( 2, $this->region, $id=false, $char=false, $realm, $guild );//
	return $this->getArmoryDataXML($url);
}
function getTalentData($base_url, $realm, $name) {
	$url = $this->_makeUrl( 4, $this->region, $id=false, $char=false, $realm, $guild );//
