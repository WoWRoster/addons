...
class ArmorySync extends ArmorySyncBase {
    ...
    function _getRosterData() {
       ...
       // //echo $this->arraydisplay($this->data);
    }
    ...
    function _getGuildInfo() {
        ...
        $content = $armory->getguilddata( $roster->data['guild_name'], $this->region, $this->server, $fetch_type='array' );//$armory->fetchGuild( $this->memberName, $roster->config['locale'], $this->server );
    ...
    function _getEquipmentInfo( $equip = array() ) {
		...
		else
		{
            ...
            $d = $armory->getArmoryDataXML($url);                                   // add item name and icon info here...
     ...
     */
      function _getReputationInfo() 
      {
		...
		else
		{
            ...
            $content =  $armory->getCharacterRep($base_url, $this->server,$this->memberName);    
    ...
    function _getTalentInfo() 
    {
		...
		else
		{
            ...
            $content = $armory->getTalentData($base_url, $this->server, $this->memberName );
		...
		$ast->errormessages = &$this->errormessages;
            ...
            require_once ($addon['dir'] . 'inc/talenticons_'.$clas. '.php');
