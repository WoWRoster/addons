...
class rpc
{
	 ...
	 */
	function rpc( $tablename, $where, $prefix='config_' )
	{
		global $roster;
	...
	function buildConfigMenu()
	{
		...
		$menu = '<!-- Begin Config Menu -->' . "\n"
			  ...
			  . '  <ul id="' . $this->prefix . 'config" class="tab_menu">' . "\n";
		...
		if (is_array($this->db_values['menu']))
		{
			foreach($this->db_values['menu'] as $values)
			{
				...
				switch ($menu_type[0])
				{
					...
					case 'makelink':
						$menu .= '    <li><a href="' . makelink('&amp;loot='.$values['value'].'') . '"' . $this->createTip($values['description'],$values['tooltip'],$values['description']) . '</a></li>' . "\n";
						break;
		...
		foreach($this->db_values[$block] as $values)
		{
			...
			switch ($input_type[0])
			{
				...
				case 'radio2':
					...
					{
						$vals = explode('^',$value);
						$input_field .= '<input type="radio" id="rad_' . $this->prefix . $this->radio_num . '" name="' . $this->prefix . $this->nospaceud($values['name']) . '_radio" value="' . $vals[1] . '" ' . ( $values['value'] == $vals[1] ? 'checked="checked"' : '' ) . ' /><label for="rad_' . $this->prefix . $this->radio_num . '" class="' . ( $values['value'] == $vals[1] ? 'blue' : 'white' ) . '">' . $vals[0] . "</label>\n";
						$this->radio_num++;
					}
					break;
	...
	function processData( &$config )
	{
		...
		foreach( $_POST as $settingName => $settingValue )
		{
			...
			$settingValue = stripslashes($settingValue);
                  $settingName = stripslashes($settingName);
			if( substr($settingName,0,strlen($this->prefix)) == $this->prefix )
			{
				// Get rid of the prefix
				$settingName = substr($settingName,strlen($this->prefix));
				//echo 'ioi '.$settingName.' = '.$settingValue.'<br>';
                        
                        
                        //update radio2 shhh this is kinda cool.....
                        //if ($settingName){
                        $per_setting = explode('_',$settingName);
                        if (is_array($per_setting) && isset($per_setting[1])){
                        //echo $per_setting[0].' - '.$per_setting[1].'-'.$settingName.'=-<br>';
                        if ($per_setting[1] == 'radio'){
                        if ($settingName == ''.$per_setting[0].'_radio')
                        {
                        //echo 'prepairing to post radio<br>';
                        //echo '*'.$this->db_values['all'][$this->nospacedu($per_setting[0])]['name'].' - '.$this->db_values['all'][$this->nospacedu($per_setting[0])]['value'].' - '.$settingValue.'*<br>';
                              if (addslashes($settingValue) != $this->db_values['all'][$this->nospacedu($per_setting[0])]['value']) 
                              {
                             // echo '<pre>';
			//	//print_r ($this->db_values['all'][$per_setting[0]]);
				//echo '</pre>';
                              //echo ''.$per_setting[0].'*'.$this->db_values['all'][$this->nospacedu($per_setting[0])]['value'].' - '.$settingValue.'*<br>';
                                    $update_sql[] = "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `config_value` = '" . $roster->db->escape($settingValue) . "' WHERE `config_name` = '" . addslashes($this->nospacedu($per_setting[0])) . "';";
                                    //echo "++UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `config_value` = '" . $roster->db->escape($settingValue) . "' WHERE `config_name` = '" . $this->nospacedu($per_setting[0]) . "'<br><hr><br>";
                        ...
                        //update percents shhh this is kinda cool.....
                        //if ($settingName){
                        $per_setting = explode('_',$settingName);
                        if (is_array($per_setting) && isset($per_setting[1])){
                        //echo $per_setting[0].' - '.$per_setting[1].'-=-<br>';
                        if ($per_setting[1] == 'percent'){
                        if ($settingName == ''.$per_setting[0].'_percent')
                        {
                        //echo '*'.$this->db_values['all'][$per_setting[0]]['percent'].' - '.$settingValue.'*<br>';
                              if ($settingValue != $this->db_values['all'][$this->nospacedu($per_setting[0])]['percent']) 
                              {
                                    $update_sql[] = "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `percent` = '" . $roster->db->escape($settingValue) . "' WHERE `config_name` = '" . addslashes($this->nospacedu($per_setting[0])) . "';";
                                    //echo "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `percent` = '" . $roster->db->escape($settingValue) . "' WHERE `config_name` = '" . $this->nospacedu($per_setting[0]) . "'--<br>";
				...
				//echo $settingName.'--[-<br>';
				//echo '<pre>';
				//print_r ($this->db_values['all'][$settingName]);
				//echo '</pre>';
                        //echo $this->db_values['all'][$settingName]['id'].' -:';
                        if ($this->db_values['all'][$settingName]['form_type'] != 'radio2' && $this->db_values['all'][$settingName]['form_type'] != 'percent'){
				if( !empty($this->db_values) && isset($this->db_values['all']) && isset($this->db_values['all'][$settingName]))
				{
				//echo 'is set - '.$this->db_values['all'][$settingName]['value'].' - "'.$this->db_values['all'][$settingName]['percent'].'" '.$settingValue.' - '.$settingName.'<br>';
				/*
				      if( $this->db_values['all'][$settingName][''.$settingName.'_percent'] != $settingValue )
					{
						$update_sql[] = "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `config_value` = '" . $roster->db->escape($settingValue) . "' WHERE (" . $this->where . ") AND `config_name` = '" . $roster->db->escape($settingName) . "';";
						echo "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `config_value` = '" . $roster->db->escape($settingValue) . "' WHERE (" . $this->where . ") AND `config_name` = '" . $roster->db->escape($settingName) . "';";
					}
				*/	
					
					if( $this->db_values['all'][$settingName]['value'] != $settingValue )
					{
						$update_sql[] = "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `config_value` = '" . $roster->db->escape($settingValue) . "' WHERE (" . $this->where . ") AND `config_name` = '" . $this->nospacedu($roster->db->escape($settingName)) . "';";
						//echo "UPDATE `" . $roster->db->table('config',$addon['basename']) . "` SET `config_value` = '" . $roster->db->escape($settingValue) . "' WHERE (" . $this->where . ") AND `config_name` = '" . $this->nospacedu($roster->db->escape($settingName)) . "<br>";
						//echo ''.$this->db_values['all'][$settingName]['value'].'<br>';
					}
					if( $this->db_values[$settingName]['value'] == $config[$settingName] )
					{
	...
	function getConfigData()
	{
		...
		if( $results && $roster->db->num_rows($results) > 0 )
		{
			while($row = $roster->db->fetch($results))
			{
				...
				$this->db_values[$setitem][$arrayitem]['percent'] = $row['percent'];
				$this->db_values[$setitem][$arrayitem]['downed'] = $row['downed'];
				...
				else
				{
					...
					$this->db_values[$setitem][$arrayitem]['description'] = $row['config_name'];
					$this->db_values[$setitem][$arrayitem]['tooltip'] = $row['config_name'] . ' From ' . $row['downed']. $db_val_line;
				}
