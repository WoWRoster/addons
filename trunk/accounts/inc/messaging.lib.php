 ...
 * @package    Accounts 
 * @subpackage Accounts Messaging Handler 
 */
...
/*
	Sample:
	$accounts->message = new accountsMessaging();
	$accounts->message->SendMessage('Promotion','Today we have a brand new promotion from the pack number5',10,30,3);//user 10 sends a message to user 30 and have a level of 3
	$array = $accounts->message->GetAllMessages(0,30);
*/

class accountsMessaging
{
	var $err_msg;
	 ...
	 *	@return accountsMessaging	returns
	 */
	function accountsMessaging()
	{
	...
	*/
	function SendMessage($title ,$body ,$sender ,$receiver ,$senderLevel)
	{
		...
		$result = $roster->db->query("INSERT INTO `".$accounts->db['message']."` (title,body,sender,uid,senderLevel) VALUES ('$title','$body',$sender,$receiver,$senderLevel)");
		if($result)
		{
			$this->err_msg = 0;
		}
	...
	*/
	function GetTitle($msgId)
	{
		...
		$result = $roster->db->query("SELECT `title` FROM `".$accounts->db['message']."` WHERE `msgid`=$msgId");
	...
	*/
	function GetBody($msgId)
	{
		...
		$result = $roster->db->query("SELECT `body` FROM `".$accounts->db['message']."` WHERE `msgid`=$msgId");
		if($roster->db->num_rows($result) == 0)
		{
			$this->err_msg = 2;
		}
	...
	*/
	function GetSenderID($msgId)
	{
		...
		$result = $roster->db->query("SELECT `sender` FROM `".$accounts->db['message']."` WHERE `msgid`=$msgId");
		if($roster->db->num_rows($result) == 0)
		{
			$this->err_msg = 2;
		}
	...
	*/
	function GetReceiverID($msgId)
	{
		...
		$result = $roster->db->query("SELECT `uid` FROM `".$accounts->db['message']."` WHERE `msgid`=$msgId");
		if($roster->db->num_rows($result) == 0)
		{
			$this->err_msg = 2;
		}
	...
	*/
	function GetSendDate( $msgId )
	{
		...
		$result = $roster->db->query("SELECT `date` FROM `".$accounts->db['message']."` WHERE `msgid`=$msgId");
		if($roster->db->num_rows($result) == 0)
		{
			$this->err_msg = 2;
		}
	...
	*/
	function GetMessage($msgId)
	{
		...
		$result = $roster->db->query("SELECT * FROM `".$accounts->db['message']."` WHERE `msgid`=$msgId");
		if($roster->db->num_rows($result) == 0)
		{
			$this->err_msg = 2;
		}
	...
	*/
	function markAsRead($msgId)
	{
		...
		$result = $roster->db->query("UPDATE `".$accounts->db['message']."` SET `read`=1 WHERE `msgid`=$msgId");
		if($result)
		{
			return;
		}
	...
	*/
	function GetAllMessages($order = 5, $receiver = '', $sender = '')
	{
		...
		$result = $roster->db->query("SELECT * FROM `".$accounts->db['message']."` WHERE $where ORDER BY $order");
	...
	*/
	function DeleteMessage($msgId)
	{
		...
		$result = $roster->db->query("DELETE FROM `".$accounts->db['message']."` WHERE `idMsg`=$msgId");
		if($result)
		{
			$this->err_msg = 0;
		}
