...
class memberslist {
	...
	function makeFilterBox()
	{
		...
		$output .=
			'<div id="sortfilterCol" style="display:'.(($roster_conf['openfilter'])?'none':'inline').';">'."\n".
			border('sblue','start',"<div style=\"cursor:pointer;width:440px;\" onclick=\"swapShow('sortfilterCol','sortfilter')\"><img src=\"".$roster_conf['img_url']."plus.gif\" style=\"float:right;\" alt=\"+\"/>".$wordings[$roster_conf['roster_lang']]['memberssortfilter']."</div>")."\n".
			border('sblue','end')."\n".
			'</div>'."\n".
			'<div id="sortfilter" style="display:'.(($roster_conf['openfilter'])?'inline':'none').';">'."\n".
			border('sblue','start',"<div style=\"cursor:pointer;width:440px;\" onclick=\"swapShow('sortfilterCol','sortfilter')\"><img src=\"".$roster_conf['img_url']."minus.gif\" style=\"float:right;\" alt=\"-\"/>".$wordings[$roster_conf['roster_lang']]['memberssortfilter']."</div>")."\n".
			'<table><tr>'."\n".
			'<td class="membersHeader">'.$wordings[$roster_conf['roster_lang']]['memberssort'].'</td>'."\n".
			'<td class="membersHeader">'.$wordings[$roster_conf['roster_lang']]['memberscolshow'].'</td>'."\n".
			'<td class="membersHeader">'.$wordings[$roster_conf['roster_lang']]['membersfilter'].'</td>'."\n".
			'<tr><td rowspan="'.$cols.'">'."\n";
		for ($i=0; $i<4; $i++) {
