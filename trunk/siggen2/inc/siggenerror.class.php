...
class SigGenError
{
	...
	function Siggen_Error( $error_level=E_ALL )
	{
		if( !$this->active )
		{
	...
	function handler( $errno , $errmsg , $filename , $linenum , $vars='' )
	{
		$errortype = array (
			...
			E_USER_ERROR      => 'SigGen Error',
			E_USER_WARNING    => 'SigGen Warning',
			E_USER_NOTICE     => 'SigGen Notice',
...
//			header('Content-type: image/' . $siggen->theme['imageformat']);
			header('Content-type: image/gif');

/*			switch ( $siggen->theme['imageformat'] )
			{
				case 'jpg':
					imagejpeg($im);
					break;
