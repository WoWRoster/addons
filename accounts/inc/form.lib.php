 ...
 * @package    Accounts 
 * @subpackage Accounts Form Handler 
 */
...
class accountsForm extends accounts
{
      var $method;
      var $action;
      var $name;
	...
	function addTextArea($name,$value,$description,$required=0,$form)
	{
		...
		$element['required'] = $required;
		$element['code'] = "<textarea name='{$element['name']}' class='{$element['class']}' rows='4' cols='1'>{$element['value']}</textarea>";
		return $this->addElement($element,$form);
	}
	...
	function addDateSelect($name,$description,$required=0,$form)
	{
		...
		$month = $roster->locale->act['acc_form']['month'];
	...
	function addTimeZone($name,$description,$required=0,$form)
	{
		...
		$selectId = $this->addSelect($name,$description,$required,$form);
			$this->addSelect_Item($selectId, '-11', $roster->locale->act['acc_form']['zone']['MIT'],$form);
			$this->addSelect_Item($selectId, '-10', $roster->locale->act['acc_form']['zone']['HST'],$form);
			$this->addSelect_Item($selectId, '-9', $roster->locale->act['acc_form']['zone']['AST'],$form);
			$this->addSelect_Item($selectId, '-8', $roster->locale->act['acc_form']['zone']['PST'],$form);
			$this->addSelect_Item($selectId, '-7', $roster->locale->act['acc_form']['zone']['PNT'],$form);
			$this->addSelect_Item($selectId, '-7', $roster->locale->act['acc_form']['zone']['MST'],$form);
			$this->addSelect_Item($selectId, '-6', $roster->locale->act['acc_form']['zone']['CST'],$form);
			$this->addSelect_Item($selectId, '-5', $roster->locale->act['acc_form']['zone']['IET'],$form);
			$this->addSelect_Item($selectId, '-5', $roster->locale->act['acc_form']['zone']['EST'],$form);
			$this->addSelect_Item($selectId, '-4', $roster->locale->act['acc_form']['zone']['PRT'],$form);
			$this->addSelect_Item($selectId, '-3.5', $roster->locale->act['acc_form']['zone']['CNT'],$form);
			$this->addSelect_Item($selectId, '-3', $roster->locale->act['acc_form']['zone']['AGT'],$form);
			$this->addSelect_Item($selectId, '-3', $roster->locale->act['acc_form']['zone']['BET'],$form);
			$this->addSelect_Item($selectId, '-1', $roster->locale->act['acc_form']['zone']['CAT'],$form);
			$this->addSelect_Item($selectId, '0', $roster->locale->act['acc_form']['zone']['GMT'],$form);
			$this->addSelect_Item($selectId, '1', $roster->locale->act['acc_form']['zone']['ECT'],$form);
			$this->addSelect_Item($selectId, '2', $roster->locale->act['acc_form']['zone']['EET'],$form);
			$this->addSelect_Item($selectId, '2', $roster->locale->act['acc_form']['zone']['ART'],$form);
			$this->addSelect_Item($selectId, '3', $roster->locale->act['acc_form']['zone']['EAT'],$form);
			$this->addSelect_Item($selectId, '3.5', $roster->locale->act['acc_form']['zone']['MET'],$form);
			$this->addSelect_Item($selectId, '4', $roster->locale->act['acc_form']['zone']['NET'],$form);
			$this->addSelect_Item($selectId, '5', $roster->locale->act['acc_form']['zone']['PLT'],$form);
			$this->addSelect_Item($selectId, '5.5', $roster->locale->act['acc_form']['zone']['IST'],$form);
			$this->addSelect_Item($selectId, '6', $roster->locale->act['acc_form']['zone']['BST'],$form);
			$this->addSelect_Item($selectId, '7', $roster->locale->act['acc_form']['zone']['VST'],$form);
			$this->addSelect_Item($selectId, '8', $roster->locale->act['acc_form']['zone']['CTT'],$form);
			$this->addSelect_Item($selectId, '9', $roster->locale->act['acc_form']['zone']['JST'],$form);
			$this->addSelect_Item($selectId, '9.5', $roster->locale->act['acc_form']['zone']['ACT'],$form);
			$this->addSelect_Item($selectId, '10', $roster->locale->act['acc_form']['zone']['AET'],$form);
			$this->addSelect_Item($selectId, '11', $roster->locale->act['acc_form']['zone']['SST'],$form);
			$this->addSelect_Item($selectId, '12', $roster->locale->act['acc_form']['zone']['NST'],$form);
	}
	...
	function addUserSelect($name,$description,$required=0,$form,$val)
	{
		...
		$sql = 'SELECT `uid`,`uname` FROM `' . $accounts->db['usertable'] . '`';
	...
	function getTableOfElements_1($rowColoring=0,$form,$i=1)
	{
		$outp = "";
		$outp .= "<form method='{$this->method[$form]}' action='{$this->action[$form]}' name='{$this->name[$form]}'>\n";
...
/** test code
$accounts->form = new accountsForm('post','index.php','mainBody','formClass');
$selectId = $accounts->form->addSelect('selectbox name','description of the select box',1);
$accounts->form->addSelect_item($selectId,'item1val','item1description');
$accounts->form->addSelect_item($selectId,'item2val','item2description');
$accounts->form->addTextBox('textbox_name','textbox_value','description of the text box','css_class',1);
$accounts->form->addFormText('text_name','description of the text box','css_class');
$accounts->form->addCheckBox('checkbox_name','checkbox_value','checkbox_description',1);
$accounts->form->addRadioButton('radiobutton_name','radiobutton_value','radiobutton_description',1);
$accounts->form->addTextArea('textarea_name','textarea_value','description of the textarea',1);
echo $accounts->form->getTableOfElements_1(1); //1 for row colorizing
echo $accounts->form->getTableOfElements_2(); //blank or 0 for no colorizing
/**/
