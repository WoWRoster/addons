...
{ 
    ...
    { 
        ...
        $installer->add_config("130,'user',NULL,'blockframe','menu'");
		...
		$installer->add_config("1040, 'cookie_name', 'wr_user', 'text{15|30', 'display'");
		...
		$installer->add_config("1070, 'admin_mail', 'admin@yoursite.com', 'text{15|30', 'display'");
		$installer->add_config("1080, 'admin_name', 'adminName', 'text{15|30', 'display'");
		$installer->add_config("1090, 'pass_length', '5', 'text{4|30', 'display'");
		$installer->add_config("1100, 'uname_length', '5', 'text{15|30', 'display'");
		
		// Page Permissions settings
		$installer->add_config("2000, 'use_perms', '1', 'radio{on^1|off^0', 'perms'");
		$installer->add_config("2010, 'admin_level', '3', 'text{4|30', 'perms'");
		$installer->add_config("2020, 'min_access', '1', 'text{4|30', 'perms'");
		
		// User administration settings
        $installer->add_config("3000, 'uname', '', 'text{24|30', 'user'");
		$installer->add_config("3010, 'fname', '', 'text{24|30', 'user'"); 
        $installer->add_config("3020, 'lname', '', 'text{24|30', 'user'");
		$installer->add_config("3030, 'pass', '', 'text{24|30', 'user'");
		$installer->add_config("3040, 'email', '', 'text{24|30', 'user'");
		$installer->add_config("3050, 'city', '', 'text{24|30', 'user'");
		$installer->add_config("3060, 'country', '', 'text{24|30', 'user'");
		$installer->add_config("3070, 'homepage', '', 'text{24|64', 'user'");
		$installer->add_config("3080, 'notes', '', 'text{24|64', 'user'");
		$installer->add_config("3090, 'extra_info', '', 'text{128|256', 'user'");
		...
		// Build db tables
		$installer->add_query("DROP TABLE IF EXISTS `" . $installer->table('user') . "`;"); 

        $installer->add_query("CREATE TABLE `" . $installer->table('user') . "` (
		...
		$installer->add_query("DROP TABLE IF EXISTS `" . $installer->table('user_link') . "`;"); 

        $installer->add_query("CREATE TABLE `" . $installer->table('user_link') . "` (
          `uid` INT(11) unsigned NOT NULL default '0', 
          `member_id` INT(11) unsigned NOT NULL default '0', 
          `guild_id` INT(11) unsigned NOT NULL default '0', 
          `group_id` smallint(6) NOT NULL default '1',
		  PRIMARY KEY (`uid`),
		  UNIQUE KEY `member` (`member_id`)
		  ) TYPE=MyISAM;"); 
		  
	    $installer->add_query("DROP TABLE IF EXISTS `" . $installer->table('page_perms') . "`;"); 

        $installer->add_query("CREATE TABLE `" . $installer->table('page_perms') . "` (
		...
		// Roster menu links
        $installer->add_menu_button('account_index','util');
		$installer->add_menu_button('account_register_bttn','account_menu','util-accounts-register','inv_misc_note_02');
		$installer->add_menu_button('account_user_bttn','account_menu','util-accounts-user&amp;fct=login','inv_misc_punchcards_prismatic');
        $installer->add_menu_button('account_characters','account_menu','char-accounts-characters','spell_holy_divinespirit'); 
        $installer->add_menu_button('account_guilds','account_menu','guild-accounts-guilds','inv_misc_tabardpvp_02'); 
        $installer->add_menu_button('account_realms','account_menu','realm-accounts-realms','spell_holy_lightsgrace'); 
