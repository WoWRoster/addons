...
// Do Not Change These //
$config['cgb']['search_order'] = array(3,4,5,6,7,8,18,9,27,1,2,31,28,25,10,12,11,13,14,
...
foreach ($config['cgb']['search_order'] as $catnumber)
{
...
function itemarray_merge($category)
{
	...
	if($config['cgb']['multilang'] == 1)
	{
		foreach ($roster_conf['multilanguages'] as $language)
		{
			if (isset($wordings['cgb'][$category][$language]))
			{
				$itemarray[$category] = array_merge($itemarray[$category], $wordings['cgb'][$category][$language]);
			}
		}	
	} else {
		$language = $roster_conf['roster_lang'];
		if (isset($wordings['cgb'][$category][$language]))
		{
			$itemarray[$category] = array_merge($itemarray[$category], $wordings['cgb'][$category][$language]);
		}
