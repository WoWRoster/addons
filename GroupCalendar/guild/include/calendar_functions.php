<?php
	...
	function amPmPullDown($pm, $namepre)
	{
		...
		echo "	<option value=\"0\"$am>". $roster->locale->act['timeAM'] ."</option>\n";
		echo "	<option value=\"1\"$pm>". $roster->locale->act['timePM'] ."</option>\n";
		echo "</select>";
	}
	...
	?>
		...
		function submitMonthYear() {
			...
			document.monthYear.action = "<?php echo makelink(); ?>&month=" + document.monthYear._month.value + "&year=" + document.monthYear._year.value + "&sid=";
	...
	function scrollArrows($m, $y)
	{
		...
		$s .= "<img src=\"addons/GroupCalendar/images/calendar/leftArrow.gif\" border=\"0\"></a> ";
		$s .= "<a href=\"".makelink()."&month=" . $nextmonth . "&year=" . $nextyear . "$sid\">";
		$s .= "<img src=\"addons/GroupCalendar/images/calendar/rightArrow.gif\" border=\"0\"></a>";
		...
		while($day <= $days) {
			...
			for($i=0;$i < 7; $i++) {
				...
				if($day > 0 && $day <= $days) {
					...
					$str .= "<a href=\"".makelink()."&d=".$day."&m=".$month."&y=".$year."\">".$day."</a>";
					...
					for($j=0;$j < $eventcount;$j++) {
						...
						switch ($eventdata[$day]["type"][$j]) {
							...
							default:
								...
								$image = "addons/GroupCalendar/images/calendar/Icon-".$eventdata[$day]["type"][$j].".png";
		...
		while($day <= $days) {
			...
			for($i=0;$i < 7; $i++) {
				...
				if($day > 0 && $day <= $days) {
					...
					$str .= "<a href=\"".makelink()."&m=".$month."&d=".$day."&y=".$year.$sid."\">".$day."</a>";
					...
					for($j=0;$j < $eventcount;$j++) {
						...
						switch ($eventdata[$day]["type"][$j]) {
							case "birthday":
								...
								$str .= "<img src=addons\GroupCalendar\images\calendar\Icon-Birth.jpg border=0 width=16 height=16></a>";
								break;
							//Do anything else
							default:
							// Blacknight
...
function getClassIcon($class)
{
	...
	if (!empty($class)){
	     ...
	     $icon_value = '<img class="membersRowimg" width="16" height="16" src="img/class/'.$icon_name.'.jpg" alt="'.$alt[0].'" /> ';
	}
      else
	{
	$icon_value = '';
