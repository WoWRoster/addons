...
///////////////////////////////////////////////////////////
	...
	function getGCtext($type) {
		global $roster, $addon;
		if (array_key_exists($type,$roster->locale->act['gcType'])) return $roster->locale->act['gcType'][$type];
		else if ($type != "") return $roster->locale->act['gcType']['OTHER'] . " ($type)";
		else return $roster->locale->act['gcType']['OTHER'];
	}
	...
	global $roster, $addon;
		...
		$base_path = "./";
		$filename = $path . "Icon-" . $type . ".png";
		//echo $filename;
		if (file_exists($filename))
            {
             return $path ."Icon-". $type . ".png";
             }
		else
            {
             return $path . "Icon-Unknown.png";
             }
