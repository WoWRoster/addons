...
class ApiSyncJobAjax extends ApiSyncJob {
    ...
    function startAjaxStatusUpdate( $id = 0 ) {
        ...
        if ( count( $this->errormessages ) > 0 ) {
            foreach ( $this->errormessages as $message ) {
				...
				$result .= $this->_xmlEncode(
					'errormessage', array( 'target' => 'ApiSync_error_table'), null,
					array(
        ...
        if ( $addon['config']['ApiSync_debuglevel'] > 0 && count( $this->debugmessages ) > 0 ) {
            foreach ( $this->debugmessages as $message ) {
				...
				$result .= $this->_xmlEncode(
					'debugmessage', array( 'target' => 'ApiSync_debug_table'), null,
					array(
