 ...
 *
 * Itemsets is a Roster addon that shows a list of all your guildmates above a defined level
...
$form .= '<input type="hidden" name="p" value="guild-ItemSets" />';
$form .= '<input type="hidden" name="guild" value="'.$roster->data['guild_id'].'" />';
...
for ($i = 0; $i < sizeof($all_sets); $i++) { 
	...
	$form .= '<option value="'.$all_sets[$i].'"'.$is_selected.'>'.$roster->locale->act[ $all_sets[$i] ].'</option>'; 
}
$form .= '</select></td>';
...
$form .= '<option value="" '.$is_selected.'>'. $roster->locale->act['All_Classes'] .'</option>';
$classArray = array();
...
// Get all the members above the defined level from the DB and sort by member name (Filtered by Class if selected)
$query = 'SELECT name, level, member_id, class, clientLocale FROM `'. $player_table .'` WHERE guild_id = '.$roster->data['guild_id'].' AND level >= '.$minlevel.' '.$class_where.' GROUP BY name ORDER BY class ASC, name ASC';
...
while ($row = $roster->db->fetch($result, SQL_ASSOC)) {
	...
	if ($items) {
		...
		echo '<a href="index.php?p=char-info&a=c:'. $row['member_id'] .'">'. $row['name'] .'</a><br /><nobr>'.$row['class'].' ('.$row['level'].')</nobr><br />';
