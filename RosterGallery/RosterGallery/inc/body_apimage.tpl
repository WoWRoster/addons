...
echo '
      ...
      <tr>
            <td align="center">
                  ['.$addon_cfg['rg_'.$row['catagory'].''].'] - <a href="'.makelink('&amp;edit=cat&amp;id='.$row['id'].'').'" >'.$roster->locale->act['admin']['edit'].'</a>
            <td>
      </tr>
...
<tr>
<td>
<div style="cursor:help;" onmouseover="overlib(\''.$row['disc'].'\',CAPTION,\''.$row['caption'].' '.$rating.'\');" onmouseout="return nd();"><img src="screenshots/thumbs/'.$row['file'].'-thumb.'.$row['ext'].'" width="130" height="110" alt="'.$row['file'].'"></a></div>
</td></tr><tr><td width="100%">
['.$roster_conf['rg_'.$row['catagory'].''].']<br>
'.$roster->locale->act['admin']['delimage'].'?  <input name="deletess[]" type="checkbox" id="delete" value="'.$row['id'].'">
</td></tr><tr><td width="100%">';
if ($row['approve']){
echo '<font color=green>'.$roster->locale->act['admin']['imageapproved'].'</font>';
}
if (!$row['approve']){
echo '<font color=red>'.$roster->locale->act['admin']['imagenotapproved'].'</font><input name="approve[]" type="checkbox" id="approve" value="'.$row['id'].'">';
}
echo '
