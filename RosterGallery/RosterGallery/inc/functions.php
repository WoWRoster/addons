	     ...
	     $frame3 = $addon['dir'].$wm_dir.'/frame_layer_3.png';
	      ...
	      $tempp2 = $addon['dir'].$wm_dir.'/temp2.png';
            ...
            if (filesize($sizea) <= $size ){
                  ...
                  {
                        $this->setMessage("Your uploaded file \"".$_FILES['userfile']['name']."\"must be (jpg gif png). Other file types are not allowed<BR>");
                  }
                  if (!$uploadcompleate)
                  {
                        $this->setMessage("Your uploaded file \"".$_FILES['userfile']['name']."\"must be (jpg gif png). Other file types are not allowed<BR>");
            ...
            {
                  $this->setMessage("The file ".basename( $_FILES['userfile']['name'])." is to large must be '.$size.'kb or less");
            }
	}
...
#######
	...
	#
      ...
      function getimage($id , $iflib, $iflb, $disc, $title, $path, $path2, $rating, $width, $height, $cat){
            ...
            <a href="'.$path2.'" rel="lightbox['.$cat.']" title="'.$title.' '.$rating.'" >
